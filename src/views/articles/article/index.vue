...
<!--      <el-select v-model="searchQuery.field" clearable placeholder="搜索字段" style="width:105px;">
        <el-option v-for="item in fieldsOptions" :key="item.key" :label="item.name" :value="item.key"></el-option>
      </el-select>-->
      搜索:
      ...
      <router-link :to="'/articles/article-create'">
    ...
    <el-table ref="multipleTable" :data="list" @selection-change="handleSelectionChange" border fit highlight-current-row>
      ...
      <el-table-column label="标题" align="center">
        <template slot-scope="scope">
          <router-link :to="'/articles/article-edit/'+scope.row.id">
...
<script>
import { getList,deleteData,getTypes } from '@/api/article'
const fieldsOptions = [
  { key: 'title', name: '标题' }
]
export default {
