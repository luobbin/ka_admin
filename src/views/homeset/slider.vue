<template>
  <div class="app-container">
    <el-form ref="postForm" :model="postForm" :rules="rules" size="small" label-width="130px">
      ...
      <el-form-item label="图片设置" prop="application">
        <div class="list-add"> <el-button type="primary" @click="handleCreate">添加图片</el-button></div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        application: [{ type: 'array', required: true, message: '数据必须在1-9个之间', min:1, max:9, trigger: 'change' }],
      },
      applicationRules: {
  ...
  methods: {
    ...
    submitForm() {
      this.$refs.postForm.validate(valid => {
        if (valid) {
          ...
          this.$router.push('/homeset/slider')
          this.loading = false
        } else {
    ...
    createData() {
      if(this.postForm.application.length>9){
